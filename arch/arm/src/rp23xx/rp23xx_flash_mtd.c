...
#define FLASH_TOTAL_LENGTH  CONFIG_RP23XX_FLASH_LENGTH
#define FLASH_BLOCK_COUNT   (FLASH_TOTAL_LENGTH / FLASH_BLOCK_SIZE)
#define FLASH_PAGE_COUNT    (FLASH_TOTAL_LENGTH / FLASH_PAGE_SIZE)
                                 ...
                                 size_t nbytes, uint8_t *buffer)
{
  ...
  if (offset + nbytes > FLASH_TOTAL_LENGTH)
    {
      return -EINVAL;
    }
...
struct mtd_dev_s *rp23xx_flash_mtd_initialize(void)
{
  ...
  finfo("rp23xx flash MTD: length=0x%x blocks=%d\n",
        (unsigned)FLASH_TOTAL_LENGTH, (int)FLASH_BLOCK_COUNT);
